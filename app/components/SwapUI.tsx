...
export function SwapUI({ market }: {market: string}) {
    ...
    const availableBalance = 36.94; // Example balance, can be dynamic
    const handlePercentClick = (percent: number) => {
        setQuantity((availableBalance * percent).toFixed(2));
    };
                    ...
                    <div className="flex flex-col px-3">
                        <div className="flex flex-col flex-1 gap-3 text-baseTextHighEmphasis">
                            <div className="flex flex-col gap-3">
                                <div className="flex items-center justify-between flex-row">
                                    ...
                                    <p className="font-medium text-xs text-baseTextHighEmphasis">{availableBalance} USDC</p>
                                </div>
                            </div>
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <div className="flex justify-center flex-row mt-2 gap-3">
                                <div onClick={() => handlePercentClick(0.25)} className="flex items-center justify-center flex-row rounded-full px-[16px] py-[6px] text-xs cursor-pointer bg-baseBackgroundL2 hover:bg-baseBackgroundL3">25%</div>
                                <div onClick={() => handlePercentClick(0.5)} className="flex items-center justify-center flex-row rounded-full px-[16px] py-[6px] text-xs cursor-pointer bg-baseBackgroundL2 hover:bg-baseBackgroundL3">50%</div>
                                <div onClick={() => handlePercentClick(0.75)} className="flex items-center justify-center flex-row rounded-full px-[16px] py-[6px] text-xs cursor-pointer bg-baseBackgroundL2 hover:bg-baseBackgroundL3">75%</div>
                                <div onClick={() => handlePercentClick(1)} className="flex items-center justify-center flex-row rounded-full px-[16px] py-[6px] text-xs cursor-pointer bg-baseBackgroundL2 hover:bg-baseBackgroundL3">Max</div>
                            </div>
                        </div>
                        <button
                            type="button"
                        ...
                        <div className="flex justify-between flex-row mt-1">
                            <div className="flex flex-row gap-2">
                                <div className="flex items-center">
                                    <input
                                        className="form-checkbox rounded border border-solid border-baseBorderMed bg-base-950 font-light cursor-pointer h-5 w-5"
                                        id="postOnly"
                                        type="checkbox"
                                        checked={isPostOnly}
                                        onChange={() => setIsPostOnly(!isPostOnly)}
                                    />
...
function BuyButton({ activeTab, setActiveTab }: { activeTab: string, setActiveTab: any }) {
    return (
        <div
            className={`flex flex-col flex-1 cursor-pointer justify-center border-b-2 p-4 ${activeTab === 'buy' ? 'border-b-greenBorder bg-greenBackgroundTransparent' : 'border-b-baseBorderMed hover:border-b-baseBorderFocus'}`}
...
function SellButton({ activeTab, setActiveTab }: { activeTab: string, setActiveTab: any }) {
    return (
        <div
            className={`flex flex-col flex-1 cursor-pointer justify-center border-b-2 p-4 ${activeTab === 'sell' ? 'border-b-redBorder bg-redBackgroundTransparent' : 'border-b-baseBorderMed hover:border-b-baseBorderFocus'}`}
